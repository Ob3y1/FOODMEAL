...
include 'connect.php';
$id1=$_GET['id'];
$sql="SELECT `ID`, `name`, `de_img` FROM `department` WHERE`ID`=$id1";
$res=$db->query($sql);
foreach($res as $row )
{
echo"

                <p style=' margin-left: 65px;'><b> Edit Part</b></p>
                <form action='modify4.php' method='post' name='form_insert' id='insert'enctype='multipart/form-data'>
                 <label>ID</label>  <input type='number' name='ID' value='$row[ID]' readonly id=''><br>
                    ...
                    <label for='input_photo'>Photo </label><input type='file' name='pro_image' placeholder='photo'
                        id='input_photo' class='upload-box' value='$row[de_img]' required><br>
